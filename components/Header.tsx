...
const Header: React.FC<HeaderProps> = ({ navLinks }) => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled || isOpen ? 'bg-black bg-opacity-90 backdrop-blur-sm' : 'bg-transparent'}`}>
      <nav className="container mx-auto px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          <a href="#home" className="absolute left-1/2 -translate-x-1/2" aria-label="BEAUZEAD Home">
            <img 
      ...
      <div className={`md:hidden transition-all duration-500 ease-in-out ${isOpen ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0'} overflow-hidden`}>
        <div className="flex flex-col items-center space-y-4 pt-4 pb-8 bg-black bg-opacity-90">
          {navLinks.map((link) => (
